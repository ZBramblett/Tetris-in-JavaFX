...
public class TetrisBoard {
    ...
    public void rotateShape(Tetrimino shape) {
        int rows = shape.getShape().length;
        int cols = shape.getShape()[0].length;
        
        int[][] rotatedPiece = new int[cols][rows];
        
        for (int pieceRow = 0; pieceRow < rows; pieceRow++) {
            for (int pieceCol = 0; pieceCol < cols; pieceCol++) {
                rotatedPiece[pieceCol][rows - 1 - pieceRow] = shape.getShape()[pieceRow][pieceCol];
            }
        }
        
        shape.setShape(rotatedPiece);
    }
